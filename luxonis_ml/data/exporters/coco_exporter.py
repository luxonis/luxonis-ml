...
class CocoExporter(BaseExporter):
    ...
    def supported_annotation_types() -> List[str]:
        return ["boundingbox", "segmentation"]
    ...
    def transform(self, prepared_ldf) -> Dict[str, Dict[str, Any]]:
        annotation_splits = {split: [] for split in self.get_split_names().keys()}
        ...
        for group_id, group_df in prepared_ldf.grouped_df:
            ...
            for row in group_df.iter_rows(named=True):
                ...
                source_to_file = {
                    name: str(
                        (Path(
                            "images") / f"{image_indices.setdefault(Path(f), len(image_indices))}{Path(f).suffix}").as_posix()
                    )
