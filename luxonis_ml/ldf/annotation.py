...
class DatasetRecord(BaseModelExtraForbid):
    ...
    annotation: Detection | list[Detection] | None = None
    task_name: str = ""
    sample_metadata: Params = Field(default_factory=dict)

    def _annotations(self) -> list[Detection]:
        """Return this record's detections as a uniform list.

        ``annotation`` may be a single `Detection`, a list of them, or
        ``None``; this normalizes all three to a list for iteration.
    ...
    def to_parquet_rows(self) -> Iterable[ParquetRecord]:
        ...
        A record may carry a single `Detection` or a list of them; either
        way the detections are flattened into the same parquet rows, so the
        on-disk format does not depend on how the record was grouped. Each
        secondary media source still receives exactly one null-annotation
        row (not one per detection).
