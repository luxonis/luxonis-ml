...
class VectorDBAPI(ABC):
    ...
    @abstractmethod
    def insert_embeddings(self, ids: List[str], embeddings: List[List[float]], payloads: List[Dict[str, Any]], batch_size: int = 100) -> None:
        """
    ...
    @abstractmethod
    def get_similarity_scores(self, reference_id: str, other_ids: List[str], sort_distances: bool = True) -> Tuple[List[str], List[float]]:
        """
    ...
    @abstractmethod
    def retrieve_payloads_by_ids(self, ids: List[str], properties: Optional[List[str]]) -> List[Dict[str, Any]]:
        """
