...
class Augmentations:
    ...
    def _parse_cfg(self, image_size: list, augmentations: dict, keep_aspect_ratio: bool = True):
    ...
    def __call__(self, data: tuple):
        ...
        transformed_image, out_bboxes, transformed_mask, final_keypoints = \
            post_augment_process(transformed, keypoints,
                keypoints_classes, use_rgb=self.train_rgb)
...
class TrainAugmentations(Augmentations):
    def __init__(self, image_size: list, augmentations: dict, train_rgb: bool = True, keep_aspect_ratio: bool = True):
        """ Class for train augmentations"""
        super().__init__(train_rgb=train_rgb)
        self.transform = self._parse_cfg(
            image_size=image_size, augmentations=augmentations, keep_aspect_ratio=keep_aspect_ratio
        )
