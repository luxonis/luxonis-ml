...
class LabelStudioConnector():
    ...
    def pull(
        ...
        for task in project.export_tasks():
            ...
            component_name = filepath.split('/')[-2] # TODO: maybe update this, as it's a bit hacky
            addition = {component_name: {'filepath': filepath}}

            cls = []
            boxes = []
            segmentation = None
            seg_name_to_class = {v:k for k,v in self.dataset.fo_dataset.mask_targets['segmentation'].items()}
            keypoints = []
                ...
                for ann in annotation:
                    ...
                    if ann['type'] == 'brushlabels':
                        ...
                        if segmentation is None:
                            segmentation = np.zeros((ann['original_height'], ann['original_width']))
