...
class LuxonisVersion:
    def __init__(self, dataset, version_number=None, samples=None, note=None):
        ...
        self.version_number = (
            dataset.version if version_number is None else version_number
        )
    ...
    def _exists(self):
        return bool(
            list(
                self.conn.version_document.find(
                    {
                        "$and": [
                            {"dataset_id_str": self.dataset_id_str},
                            {"number": self.version_number},
    ...
    def _find(self):
        return self.conn.version_document.find(
            {"dataset_id_str": self.dataset_id_str, "number": self.version_number}
        )[0]
