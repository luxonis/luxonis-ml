...
class LuxonisParser:
    def __init__(self, **ldf_kwargs):
        if LuxonisDataset.exists(ldf_kwargs["dataset_name"]):
            dataset = LuxonisDataset(ldf_kwargs["dataset_name"])
    ...
    def parsing_wrapper(func):
        def wrapper(*args, **kwargs):
            ...
            dataset.make_splits()  # TODO: remove from here
            return dataset

        return wrapper

    @parsing_wrapper
    def from_coco_format(self, image_dir: str, annotation_path: str):
    ...
    @parsing_wrapper
    def from_create_ml_format(self, image_dir: str, annotation_path: str):
    ...
    @parsing_wrapper
    def from_tensorflow_csv_format(self, image_dir: str, annotation_path: str):
        with open(annotation_path) as f:
    ...
    @parsing_wrapper
    def from_seg_mask_format(self, image_dir: str, seg_dir: str, annotation_path: str):
        with open(annotation_path) as f:
            reader = csv.reader(f, delimiter=",")
        ...
        def generator():
            ...
            for image_path in images:
                ...
                for id in ids:
                    ...
                    polylines = [
                        np.array(polygon).squeeze().tolist()
                        for polygon in contours
                        if len(polygon) > 1
                    ]

                    for polyline in polylines:
                        # NOTE: contours are strange
                        yield {
                            "file": path,
                            "class": class_name,
                            "type": "polyline",
