...
class LuxonisDataset:
    ...
    def from_coco(self, source_name, image_dir, annotation_path, split='train', override_main_component=None):
        """
        ...
        if override_main_component is not None:
            ...
            component_name = self.sources[source_name].main_component
        ...
        for image in tqdm(images):
            ...
            if os.path.exists(image_path):
                ...
                for ann in anns:
                    ...
                    class_id = self._add_class(class_name)
                    if cat_id in keypoint_definitions.keys():
                        self._add_keypoint_definition(class_id, keypoint_definitions[cat_id])
    ...
    def from_yolo(self, source_name, yaml_path, split='all', override_main_component=None):
        """
        source_name: name of the LDFSource to add to
