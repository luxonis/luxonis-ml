...
def test_grid_hits_maps_each_tile_to_composite_pixels() -> None:
    ...
    composite, hits = grid_hits([a, b], ncols=2, pad=10)
...
def test_combine_hits_threads_through_nesting() -> None:
    ...
    b, _ = _tile("B")
    c, _ = _tile("C")
    composite, hits = combine_hits(a, [b, c])
...
def test_combine_hits_titled_mapping() -> None:
    a, _ = _tile("A")
    b, _ = _tile("B")
    composite, hits = combine_hits({"left": a, "right": b})
...
def test_fit_grid_matches_grid_hits_when_unscaled() -> None:
    a, _ = _tile("A")
    b, _ = _tile("B")
    fit_img, fit_hits = fit_grid([a, b], target=(10_000, 10_000), ncols=2)
    grid_img, _ = grid_hits([a, b], ncols=2)
...
def test_fit_grid_scales_within_target() -> None:
    a, _ = _tile("A")
    b, _ = _tile("B")
    composite, hits = fit_grid([a, b], target=(150, 100), ncols=2)
