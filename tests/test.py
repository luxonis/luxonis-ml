...
class LuxonisDatasetTester(unittest.TestCase):
    ...
    def test_source(self):
        ...
        res2 = list(curr)[0]
        self.assertEqual(
            res2["component_itypes"], [1, 1], "Wrong IType after changing source"
        )
    ...
    def test_add(self):
        with LuxonisDataset(self.team_id, self.dataset_id) as dataset:
            ...
            self.assertEqual(
                len(transaction_to_additions),
                30,
                "Wrong number of entries in transaction_to_additions",
            )
            ...
            num_updates = np.sum(
                [True if t["action"] == "UPDATE" else False for t in res]
            )
            num_deletes = np.sum(
                [True if t["action"] == "DELETE" else False for t in res]
            )
            ...
            res = list(curr)
            num_executed = np.sum(
                [True if t["executed"] == True else False for t in res]
            )
        ...
        with LuxonisDataset(self.team_id, self.dataset_id) as dataset:
            ...
            for sample in dataset.fo_dataset:
                ...
                if sample.version == 1.0:
                    num_version += 1
            self.assertEqual(
                num_latest, 15, "Not all versioned samples are latest for A"
            )
            self.assertEqual(
                num_version,
                15,
                "Not all versioned samples have the right version for A",
            )
            ...
            for sample in dataset.fo_dataset:
                ...
                if sample.version == 1.0:
                    num_version += 1
            self.assertEqual(
                num_latest, 15, "Not all versioned samples are latest for B"
            )
            self.assertEqual(
                num_version,
                15,
                "Not all versioned samples have the right version for B",
            )
    ...
    def test_modify(self):
        with LuxonisDataset(self.team_id, self.dataset_id) as dataset:
            ...
            self.assertEqual(
                field_change,
                True,
                "Keypoints update (change coordinate non-NaN->non-NaN)",
            )
            a = deepcopy(self.additions[-1])
            a["B"]["keypoints"][0][1][8] = (
                a["A"]["keypoints"][0][1][0][0] + 0.1,
            ...
            transaction_to_additions += tta
            self.assertEqual(
                field_change, True, "Keypoints update (change coordinate NaN->non-Nan)"
            )
            a = deepcopy(self.additions[-1])
            ...
            transaction_to_additions += tta
            self.assertEqual(
                field_change, True, "Keypoints update (change coordinate non-NaN->NaN)"
            )
            ...
            self.assertEqual(
                ...
                "orange",
                "Executed change to classification label",
            )
            # TODO: better box test
            self.assertEqual(
                np.sum(sample["segmentation"]["mask"]),
                0.0,
                "Executed change to segmentation mask",
            )
            ...
            self.assertEqual(
                len(query), 1, "Only one change in B when executing modifications"
            )
            for sample in query:
                break
            # TODO: keypoint test
    ...
    def test_version_2(self):
        with LuxonisDataset(self.team_id, self.dataset_id) as dataset:
            ...
            self.assertEqual(
                num_version, 1, "Not all versioned samples have the right version for A"
            )
            ...
            self.assertEqual(
                num_version, 1, "Not all versioned samples have the right version for B"
            )
        ...
        with LuxonisDataset(self.team_id, self.dataset_id) as dataset:
            ...
            for sample in dataset.fo_dataset:
                ...
                if sample.version == 2.1:
                    num_version += 1
            self.assertEqual(
                num_latest, 14, "Not all versioned samples are latest for A"
            )
            self.assertEqual(
                num_version, 0, "Not all versioned samples have the right version for A"
            )
            ...
            for sample in dataset.fo_dataset:
                ...
                if sample.version == 2.1:
                    num_version += 1
            self.assertEqual(
                num_latest, 14, "Not all versioned samples are latest for B"
            )
            self.assertEqual(
                num_version, 0, "Not all versioned samples have the right version for B"
            )
    ...
    def test_delete_dataset(self):
        ...
        curr = self.conn.luxonis_source_document.find({"_luxonis_dataset_id": old_id})
        res2 = list(curr)
